...
@click.option('--screenshot', is_flag=True, help='生成多设备截图')
@click.option('--serve', is_flag=True, help='启动本地 HTTP 服务')
@click.option('--port', '-p', default=8080, help='服务端口（默认：8080）')
def cli(content_or_file, color, style, device, output, name, template, screenshot, serve, port):
    """生成 HTML 信息图"""
    ...
    if screenshot:
        ...
        try:
            import subprocess
            screenshot_script = Path(__file__).parent / 'screenshot.py'
            if screenshot_script.exists():
                subprocess.run([
                    sys.executable, str(screenshot_script),
                    result['html_path'],
                    '--all-portrait',
                    '-o', os.path.join(result['project_path'], 'screenshots')
                ], check=True)
                click.echo(f"✅ 截图已保存到：{os.path.join(result['project_path'], 'screenshots/')}")
            else:
                click.echo(f"⚠️  截图脚本不存在：{screenshot_script}")
    ...
    if serve:
        ...
        try:
            ...
            qr_path = project_path / 'qrcode.png'
            ...
            click.echo(f"📱 局域网访问：http://{local_ip}:{port}")
            click.echo(f"📱 手机扫码查看：{qr_path}")
            click.echo(f"📁 服务目录：{project_path}")
            click.echo()
            ...
            click.echo(f"⚠️  缺少依赖：pip install qrcode[pil]")
            click.echo(f"   错误：{e}")
        except KeyboardInterrupt:
            click.echo(f"\n✅ 服务已停止")
        except Exception as e:
            click.echo(f"⚠️  服务启动失败：{e}")
